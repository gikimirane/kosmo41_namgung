...
public class WsServer2 {
	...
	//해쉬맵 역할을 해주는애
		...
		if (com.equals("!입장")) {
			...
			String tran = message.substring(message.indexOf("^")+1);
			trantype = Integer.parseInt(tran);
			
			String msg1 = usermsg.substring(0, usermsg.indexOf("^"));
			System.out.println("글자자름"+msg1);
			
			if(trantype<1) {
				message = user + " : " + msg1;
			}else {
				translate = trans.getTranslate(msg1,trantype);
				message = "@"+user + " : " + msg1+" # "+translate;
			}
			basic.sendText(message);
