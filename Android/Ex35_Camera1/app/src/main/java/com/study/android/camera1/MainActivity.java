...
public class MainActivity extends AppCompatActivity {
    ...
    private void showCaptureImage(){
        ...
        Bitmap scaleBitmap = Bitmap.createScaledBitmap(bitmap,800,800,false);
        bitmap.recycle();
    ...
    public void onConfigurationChanged(Configuration newConfig){
        ...
        if(newConfig.orientation==Configuration.ORIENTATION_PORTRAIT){
            Toast.makeText(getApplicationContext(),"세로 방향으로 변경 됨.",Toast.LENGTH_LONG).show();
            if(imageUri!=null) {
                showCaptureImage();
            }
        }else if(newConfig.orientation==Configuration.ORIENTATION_LANDSCAPE){
            Toast.makeText(getApplicationContext(),"가로 방향으로 변경 됨.",Toast.LENGTH_LONG).show();
            if(imageUri!=null){
                showCaptureImage();
            }
        }
